function formatLocalDate(date) {
  const year = date.getFullYear();
  const month = String(date.getMonth() + 1).padStart(2, "0");
  const day = String(date.getDate()).padStart(2, "0");
  return `${year}-${month}-${day}`;
}

export function normalizeDate(date) {
  if (!date) {
    return "";
  }

  if (typeof date === "string") {
    const trimmed = date.trim();
    if (!trimmed) {
      return "";
    }

    const isoPrefixMatch = trimmed.match(/^(\d{4})-(\d{2})-(\d{2})/);
    if (isoPrefixMatch) {
      const [, year, month, day] = isoPrefixMatch;
      return `${year}-${month}-${day}`;
    }

    const parsedFromString = new Date(trimmed);
    if (Number.isNaN(parsedFromString.getTime())) {
      return "";
    }

    return formatLocalDate(parsedFromString);
  }

  const parsed = date instanceof Date ? date : new Date(date);
  if (Number.isNaN(parsed.getTime())) {
    return "";
  }

  return parsed.toISOString().split("T")[0];
}

export function filterByDate(data, filter, key) {
  const { from, to } = getDateBounds(filter);

  return data.filter((entry) => {
    const entryDate = normalizeDate(entry[key] || entry.created_at);
    return entryDate && entryDate >= from && entryDate <= to;
  });
}

// Inclusive day count between two YYYY-MM-DD strings. Returns at least 1 when both dates are equal.
export function dateDiffDaysInclusive(from, to) {
  const a = normalizeDate(from);
  const b = normalizeDate(to);
  if (!a || !b) return 0;
  const ms = new Date(`${b}T12:00:00Z`) - new Date(`${a}T12:00:00Z`);
  return Math.round(ms / 86400000) + 1;
}

// Keep expenses whose coverage window [covers_from, covers_to] overlaps the [from, to] window at all.
// Falls back to spent_on for legacy rows that haven't been re-normalized yet.
export function filterExpensesByWindow(expenses, filter) {
  const { from, to } = getDateBounds(filter);
  return (expenses || []).filter((expense) => {
    const cf = normalizeDate(expense.covers_from || expense.spent_on);
    const ct = normalizeDate(expense.covers_to || expense.covers_from || expense.spent_on);
    if (!cf || !ct) return false;
    return cf <= to && ct >= from;
  });
}

// For each expense, attach the share that belongs inside [from, to]:
//   daily_share   = amount / span_days
//   overlap_days  = inclusive days of [covers_from, covers_to] ∩ [from, to]
//   allocated_amount = daily_share × overlap_days
// This is what makes a GH₵500 / 10-day stock purchase show as GH₵50/day instead of −400 on day 1.
export function allocateExpensesToWindow(expenses, filter) {
  const { from, to } = getDateBounds(filter);
  return (expenses || []).map((expense) => {
    const coversFrom = normalizeDate(expense.covers_from || expense.spent_on);
    const coversTo = normalizeDate(expense.covers_to || expense.covers_from || expense.spent_on);
    const amount = Number(expense.amount || 0);
    const spanDays = Math.max(1, dateDiffDaysInclusive(coversFrom, coversTo));
    const overlapFrom = coversFrom > from ? coversFrom : from;
    const overlapTo = coversTo < to ? coversTo : to;
    const overlapDays = overlapFrom <= overlapTo ? dateDiffDaysInclusive(overlapFrom, overlapTo) : 0;
    const dailyShare = amount / spanDays;
    return {
      ...expense,
      span_days: spanDays,
      daily_share: dailyShare,
      overlap_days: overlapDays,
      allocated_amount: dailyShare * overlapDays,
    };
  });
}

export function computeMetrics(sales, expenses) {
  const revenue = sales.reduce((total, sale) => total + Number(sale.total || 0), 0);
  // Prefer allocated_amount when the caller pre-allocated via allocateExpensesToWindow;
  // otherwise fall back to raw amount (legacy behavior, used by tests / unallocated callers).
  const expenseTotal = expenses.reduce(
    (total, expense) =>
      total +
      (expense.allocated_amount !== undefined
        ? Number(expense.allocated_amount || 0)
        : Number(expense.amount || 0)),
    0
  );
  const net = revenue - expenseTotal;
  const itemsSold = sales.reduce((total, sale) => total + Number(sale.quantity || 0), 0);
  const saleCount = sales.length;
  const expenseCount = expenses.length;
  const bestSeller = getBestSeller(sales);
  const biggestExpense = getBiggestExpense(expenses);
  const margin = revenue > 0 ? ((net / revenue) * 100) : 0;
  const bestSellerRevenue = bestSeller
    ? sales.filter((s) => s.item_name === bestSeller.name).reduce((t, s) => t + Number(s.total || 0), 0)
    : 0;

  return {
    revenue,
    expenseTotal,
    net,
    itemsSold,
    saleCount,
    expenseCount,
    bestSeller,
    bestSellerRevenue,
    biggestExpense,
    margin,
    statusTone: net > 0 ? "positive" : net < 0 ? "negative" : "neutral",
    statusLabel: net > 0 ? "Profit" : net < 0 ? "Loss" : "Break even",
  };
}

export function createSingleDateFilter(value = normalizeDate(new Date())) {
  return {
    type: "single",
    value,
    from: null,
    to: null,
  };
}

// 'All time' filter: a sentinel that callers can detect to skip date constraints in DB queries.
// getDateBounds returns extreme dates so any window-based math (e.g. expense allocation) keeps
// working without special cases — every covered day overlaps a 1970–2999 window.
export function createAllTimeFilter() {
  return {
    type: "all",
    value: "",
    from: null,
    to: null,
  };
}

export function createRangeDateFilter(from, to) {
  return {
    type: "range",
    value: "",
    from: normalizeDate(from),
    to: normalizeDate(to),
  };
}

export function getDateBounds(filter) {
  if (filter.type === "all") {
    // Wide enough to cover any realistic transaction date so overlap math behaves naturally.
    return { from: "1970-01-01", to: "2999-12-31" };
  }

  if (filter.type === "range") {
    const rawFrom = normalizeDate(filter.from);
    const rawTo = normalizeDate(filter.to || filter.from);

    if (!rawFrom || !rawTo) {
      const today = normalizeDate(new Date());
      return { from: today, to: today };
    }

    return rawFrom <= rawTo ? { from: rawFrom, to: rawTo } : { from: rawTo, to: rawFrom };
  }

  const value = normalizeDate(filter.value || new Date());
  return { from: value, to: value };
}

export function shiftDateFilter(filter, amount) {
  // 'All time' has no meaningful prev/next; tapping the arrow drops her into a single day so
  // she can navigate normally from there. Pick today as the landing day.
  if (filter.type === "all") {
    return createSingleDateFilter(shiftDateValue(normalizeDate(new Date()), amount));
  }

  if (filter.type === "range") {
    return {
      ...filter,
      from: shiftDateValue(filter.from, amount),
      to: shiftDateValue(filter.to, amount),
    };
  }

  return {
    ...filter,
    value: shiftDateValue(filter.value, amount),
  };
}

export function shiftDateValue(value, amount) {
  const normalized = normalizeDate(value || new Date());
  const date = new Date(`${normalized}T12:00:00Z`);

  if (Number.isNaN(date.getTime())) {
    return "";
  }

  date.setUTCDate(date.getUTCDate() + amount);
  return date.toISOString().split("T")[0];
}

export function createTodayFilter() {
  return createSingleDateFilter(normalizeDate(new Date()));
}

// Convenience: 'this month' range from the 1st of the current month to today.
export function createThisMonthFilter() {
  const now = new Date();
  const first = new Date(now.getFullYear(), now.getMonth(), 1);
  return createRangeDateFilter(normalizeDate(first), normalizeDate(now));
}

// Convenience: rolling last 7 days ending today.
export function createThisWeekFilter() {
  const today = normalizeDate(new Date());
  return createRangeDateFilter(shiftDateValue(today, -6), today);
}

export function createYesterdayFilter() {
  return createSingleDateFilter(shiftDateValue(normalizeDate(new Date()), -1));
}

export function normalizeSalesRows(rows) {
  return (rows || []).map((sale) => ({
    ...sale,
    sold_on: normalizeDate(sale.sold_on || sale.created_at),
    quantity: Number(sale.quantity || 0),
    unit_price: Number(sale.unit_price || 0),
    total: Number(sale.total || 0),
  }));
}

export function normalizeExpenseRows(rows) {
  return (rows || []).map((expense) => {
    const expenseName =
      (expense.expense_name || expense.name || expense.notes || expense.category || "Expense").trim() || "Expense";

    const spentOn = normalizeDate(expense.spent_on || expense.created_at);
    // Backwards compat: if covers_from/covers_to are absent (legacy row), treat as single-day on spent_on.
    const coversFromRaw = normalizeDate(expense.covers_from) || spentOn;
    const coversToRaw = normalizeDate(expense.covers_to) || coversFromRaw || spentOn;
    // Guarantee covers_to >= covers_from defensively (DB constraint enforces it server-side).
    const coversFrom = coversFromRaw <= coversToRaw ? coversFromRaw : coversToRaw;
    const coversTo = coversToRaw >= coversFromRaw ? coversToRaw : coversFromRaw;

    return {
      ...expense,
      spent_on: spentOn,
      covers_from: coversFrom,
      covers_to: coversTo,
      amount: Number(expense.amount || 0),
      category: (expense.category || "General").trim() || "General",
      expense_name: expenseName,
    };
  });
}

export function buildSalePayload(form, menuItems, soldOn) {
  const selectedMenu = menuItems.find((item) => item.id === form.itemId);
  const quantity = Math.max(1, Number.parseInt(form.quantity, 10) || 0);
  const unitPrice = Math.max(0, Number.parseFloat(form.unitPrice) || 0);

  if (!selectedMenu || quantity <= 0 || unitPrice <= 0) {
    return { error: "Enter a valid sale before saving.", payload: null };
  }

  return {
    error: "",
    payload: {
      item_id: selectedMenu.id,
      item_name: selectedMenu.name,
      sold_on: normalizeDate(soldOn),
      quantity,
      unit_price: unitPrice,
      total: quantity * unitPrice,
      notes: "",
    },
  };
}

export function buildExpensePayload(form, spentOn) {
  const amount = Math.max(0, Number.parseFloat(form.amount) || 0);
  const expenseName = (form.name || "").trim();
  const category = (form.category || "").trim();

  if (!category) {
    return { error: "Select a category before saving.", payload: null };
  }
  if (!expenseName || amount <= 0) {
    return { error: "Enter an expense name and amount before saving.", payload: null };
  }

  const spent = normalizeDate(spentOn);
  // Default covers window = single day on spent_on (matches legacy behavior).
  const coversFromRaw = normalizeDate(form.coversFrom) || spent;
  const coversToRaw = normalizeDate(form.coversTo) || coversFromRaw;
  const coversFrom = coversFromRaw <= coversToRaw ? coversFromRaw : coversToRaw;
  const coversTo = coversToRaw >= coversFromRaw ? coversToRaw : coversFromRaw;

  if (!coversFrom || !coversTo) {
    return { error: "Pick a valid coverage period before saving.", payload: null };
  }

  return {
    error: "",
    payload: {
      category,
      amount,
      spent_on: spent,
      covers_from: coversFrom,
      covers_to: coversTo,
      notes: expenseName,
    },
  };
}

// Build an autocomplete-ready history list from raw expense rows.
// Input rows must already be normalized (have expense_name, category, amount, spent_on/created_at).
// Output is one entry per unique expense_name, carrying the most-recently-used category + amount,
// sorted by recency so the most relevant suggestions appear first.
export function dedupeExpenseHistory(rows) {
  const seen = new Map();
  // Iterate in the caller's order; we assume callers pass newest-first.
  for (const row of rows || []) {
    const name = (row.expense_name || row.notes || "").trim();
    if (!name) continue;
    const key = name.toLowerCase();
    if (seen.has(key)) continue;
    seen.set(key, {
      name,
      category: row.category || "",
      amount: Number(row.amount || 0),
      lastUsed: row.spent_on || row.created_at || "",
    });
  }
  return Array.from(seen.values());
}

export function buildSalesExportRows(sales) {
  return [
    ["Date", "Item", "Quantity", "Unit Price (GH₵)", "Total (GH₵)"],
    ...sales.map((sale) => [
      sale.sold_on,
      sale.item_name,
      sale.quantity,
      sale.unit_price.toFixed(2),
      sale.total.toFixed(2),
    ]),
  ];
}

export function buildExpenseExportRows(expenses) {
  return [
    ["Date", "Expense Name", "Category", "Amount (GH₵)"],
    ...expenses.map((expense) => [
      expense.spent_on,
      expense.expense_name,
      expense.category,
      expense.amount.toFixed(2),
    ]),
  ];
}

export function parseCSVRows(text) {
  const rows = [];
  let current = "";
  let row = [];
  let inQuotes = false;

  for (let index = 0; index < text.length; index += 1) {
    const char = text[index];
    const nextChar = text[index + 1];

    if (char === '"') {
      if (inQuotes && nextChar === '"') {
        current += '"';
        index += 1;
      } else {
        inQuotes = !inQuotes;
      }
      continue;
    }

    if (char === "," && !inQuotes) {
      row.push(current.trim());
      current = "";
      continue;
    }

    if ((char === "\n" || char === "\r") && !inQuotes) {
      if (char === "\r" && nextChar === "\n") {
        index += 1;
      }

      row.push(current.trim());
      rows.push(row);
      row = [];
      current = "";
      continue;
    }

    current += char;
  }

  if (current.length || row.length) {
    row.push(current.trim());
    rows.push(row);
  }

  return rows.filter((entry) => entry.some((cell) => cell.length));
}

export function buildExpenseUploadRows(rows, fallbackDate) {
  if (!rows.length) {
    return [];
  }

  const [headerRow, ...dataRows] = rows;
  const headerMap = headerRow.reduce((accumulator, value, index) => {
    accumulator[normalizeHeader(value)] = index;
    return accumulator;
  }, {});

  const nameIndex = firstDefinedIndex(headerMap, ["expense name", "name", "notes"]);
  const categoryIndex = firstDefinedIndex(headerMap, ["category"]);
  const amountIndex = firstDefinedIndex(headerMap, ["amount"]);
  const dateIndex = firstDefinedIndex(headerMap, ["date"]);

  if (amountIndex === undefined || categoryIndex === undefined) {
    throw new Error("CSV must include Category and Amount columns.");
  }

  return dataRows
    .map((row, index) => {
      const rawName = nameIndex === undefined ? "" : (row[nameIndex] || "").trim();
      const rawCategory = (row[categoryIndex] || "").trim();
      const rawAmount = (row[amountIndex] || "").trim();
      const rawDate = dateIndex === undefined ? "" : (row[dateIndex] || "").trim();

      if (!rawName && !rawCategory && !rawAmount && !rawDate) {
        return null;
      }

      const amount = Number.parseFloat(rawAmount);
      const category = rawCategory || "General";
      const expenseName = rawName || rawCategory;

      if (!expenseName) {
        throw new Error(`Row ${index + 2}: Expense name is required.`);
      }

      if (!category) {
        throw new Error(`Row ${index + 2}: Category is required.`);
      }

      if (!Number.isFinite(amount) || amount <= 0) {
        throw new Error(`Row ${index + 2}: Amount must be greater than 0.`);
      }

      const spentOn = normalizeDate(rawDate || fallbackDate);

      if (!spentOn) {
        throw new Error(`Row ${index + 2}: Date must use YYYY-MM-DD.`);
      }

      return {
        category,
        amount,
        spent_on: spentOn,
        notes: expenseName,
      };
    })
    .filter(Boolean);
}

function normalizeHeader(value) {
  return value.toLowerCase().replace(/\s*\(.*?\)\s*/g, "").trim();
}

function firstDefinedIndex(map, keys) {
  return keys.find((key) => map[key] !== undefined) !== undefined
    ? map[keys.find((key) => map[key] !== undefined)]
    : undefined;
}

function getBiggestExpense(expenses) {
  if (!expenses.length) {
    return null;
  }

  // Use allocated_amount when present so the 'biggest' is judged by what hit *this* window,
  // not by the raw lump sum of a multi-day purchase.
  return expenses.reduce((biggest, expense) => {
    const amount =
      expense.allocated_amount !== undefined
        ? Number(expense.allocated_amount || 0)
        : Number(expense.amount || 0);
    return amount > (biggest?.amount || 0)
      ? { name: expense.expense_name || expense.category || "Expense", amount }
      : biggest;
  }, null);
}

function getBestSeller(sales) {
  if (!sales.length) {
    return null;
  }

  const grouped = sales.reduce((accumulator, sale) => {
    const key = sale.item_name || "Unknown";

    if (!accumulator[key]) {
      accumulator[key] = {
        name: key,
        quantity: 0,
      };
    }

    accumulator[key].quantity += Number(sale.quantity || 0);
    return accumulator;
  }, {});

  return Object.values(grouped).sort((left, right) => {
    if (right.quantity !== left.quantity) {
      return right.quantity - left.quantity;
    }

    return left.name.localeCompare(right.name);
  })[0];
}
