export function normalizeDate(date) {
  ...
  if (Number.isNaN(parsed.getTime())) {
    const fallback = String(date).slice(0, 10);
...
export function buildExpensePayload(form, spentOn) {
  ...
  if (!expenseName || !category || amount <= 0) {
    return { error: "Enter an expense name, category, and amount before saving.", payload: null };
  }
